          ...
          <div className="flex items-center h-full">
            {isEditingName ? (
              ...
              <div className="flex items-center justify-between w-full">
                ...
                <div className="flex items-center space-x-0 sm:space-x-1 ml-1 flex-shrink-0" style={{ paddingRight: isReadOnly ? '4px' : '0' }}>
                  <button 
                    onClick={() => setShowComments(!showComments)}
                    className={`${hasComments ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-600 relative flex-shrink-0 ${isReadOnly ? 'ml-2' : ''}`}
                    style={{ marginRight: isReadOnly ? '8px' : undefined }}
                  >
                    {hasComments && (
